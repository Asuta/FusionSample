...
namespace Fusion107
{
    ...
    public class PlayerMovement : NetworkBehaviour
    {
        ...
        private void GrabSomething(Transform HandGrabT, Rigidbody HandRb)
        {
            ...
            foreach (Collider collider in colliders)
            {
                if (collider != null && collider.GetComponent<Rigidbody>() != null && collider.tag != "unGrabable")
                {
                    ...
                    RPC_SendGrab(collider.GetComponent<NetworkObject>(), HandRb.GetComponent<NetworkObject>(), fixedJoint.anchor, fixedJoint.connectedAnchor);
                    //结束这个循环
                    break;
                }
            }
        ...
        [Rpc(RpcSources.StateAuthority, RpcTargets.All)]
        public void RPC_SendGrab(NetworkObject grabThing, NetworkObject hand, Vector3 anchorPosition, Vector3 connectedAnchorPosition, RpcInfo info = default)
        {
            if (!HasStateAuthority)
            {
        ...
        private void TakeOutSomething(Rigidbody HandRb)
        {
            ...
            RPC_SendRealse(HandRb.GetComponent<NetworkObject>());
        }


        [Rpc(RpcSources.StateAuthority, RpcTargets.All)]
        public void RPC_SendRealse(NetworkObject hand, RpcInfo info = default)
        {
            if (!HasStateAuthority)
            {
